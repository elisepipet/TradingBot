...
class Trade:
    ...
    def UpdateCandlesValues(self, line):
        ...
        if (len(self.values) < 7):
            return
        self.G.append(self.TIA([candle['close'] for candle in self.values], 7))
        self.R.append(self.RET(self.values[-7]['close'], self.values[-1]['close']))
        self.S.append(self.StandardDeviation([candle['close'] for candle in self.values]))
        return
    ...
    def getAverage(self, list):
        ...
        for i in range (1, len(list)):
            if (list[i] - list[i - 1] > 0):
                av.append(list[i] - list[i - 1])
        return av
    
    def TIA(self, list, p):
        av = self.getAverage(list)
        return sum(av) / p
    ...
    def StandardDeviation(self, list):
        ...
        for i in range (len(list)):
            res += pow(list[i] - moy, 2)
        return math.sqrt(res / len(list))
    
    def RET(self, initial, last):
        res = (last - initial) / initial * 100

        if (res > 0 and res - round(res) >= 0.5):
            if (res > 0): return round(res) + 1
            else: return round(res) - 1
        if (res > 0 and res - round(res) < 0.5):
            return round(res)
        if (res < 0 and (res * -1) - (round(res * -1)) < 0.5):
            return round(res)
        if (res < 0 and (res * -1) - (round(res * -1)) >= 0.5):
            if (res > 0): return round(res) + 1
            else: return round(res) - 1
    ...
    def GRSonCandles(self):
        pos_occurr = 0
        neg_occurr = 0
        if (self.R[-3] < 0 and self.R[-2] > 0):
            pos_occurr += 1
        if (self.R[-3] > 0 and self.R[-2] < 0):
            neg_occurr += 1
        if (self.R[-3] == 0 and self.R[-2] < 0):
        ...
        if (self.S[-1] > self.S[-2]):
            neg_occurr += 1
        if (self.G[-3] < 0 and self.G[-2] > 0):
            pos_occurr += 1
        if (self.G[-3] > 0 and self.G[-2] < 0):
            neg_occurr += 1
        if (self.G[-3] == 0 and self.G[-2] < 0):
            neg_occurr += 1
        return pos_occurr, neg_occurr
    ...
    def DetectCrashMarket(self):
        ...
        w = l = 0
        for i in range(1, 14):
            diff = recent_candles[i]['close'] - recent_candles[i - 1]['close']
            if diff > 0:
                gains += diff
            else:
                losses += abs(diff)
        w = gains / 14
        l = losses / 14
        rs = w / l if losses != 0 else float('inf')
        rsi = 100 - (100 / (1 + rs))
    ...
    def MakeADecision(self):
        nb = self.stacks['USDT'] * 0.8 / self.values[-1]['close']
        nb = round(nb, 3)
        SAver = self.AverageMobile(5)
        LAver = self.AverageMobile(10)
        p, n = self.GRSonCandles()
        avVol = self.AverageVol()
        curr_vol = self.values[-1]['volume']
        ok = self.DetectCrashMarket()

        if (self.stacks['USDT'] < self.stack and self.crash > 1):
            print("pass", flush=True)
            return

        if (((p == 0 or SAver < LAver or curr_vol < avVol) or ok == False or self.values[-1]['close'] < LAver) and self.stacks['BTC'] != 0):
            print("sell", self.pair, self.stacks['BTC'], flush=True)
        elif (nb <= 0 and ok == True):
            print("pass", flush=True)
            return
        elif (n == 0 or SAver > LAver or ok == False or self.values[-1]['close'] > LAver or curr_vol > avVol):
